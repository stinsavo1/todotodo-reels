...
@UntilDestroy()
export class VerificationCodeComponent implements ViewDidLeave, ViewWillEnter  {
  ...
  public back(): void {
    this.navCtrl.navigateRoot('/tabs/map/registration');
  }
  ...
  public resendCode(): void {
    if (!this.isSending) {
      this.isSending = true;
      this.startTimer();
      this.core.presentAlert('Уведомление', 'Новый код был отправлен по SMS. Пожалуйста, проверьте ваш телефон.');
      this.isError = false;
      this.verificationService.resendCodeSMS(this.phoneNumber).pipe(untilDestroyed(this)).subscribe();
    }
  }
  ...
  public verifyAndLogin(): void {
    if (this.code.join('').length === 4) {
      ...
      this.verificationService.verifyCodeBySms(this.phoneNumber, this.code.join('')).subscribe({
        next: () => {},
        error: err => {
          this.isError = true;
